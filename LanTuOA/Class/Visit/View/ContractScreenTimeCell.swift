...
class ContractScreenTimeCell: UITableViewCell {
    ...
    var data: (String, String, Int?)! {
        didSet {
            let titleStr = data?.0
            let placeholderStr = data?.1
            let timeStamp = data?.2
            titleLabel.text = titleStr
            if timeStamp == nil {
                screenLabel.text = placeholderStr
                screenLabel.textAlignment = .left
                ...
                screenLabel.text = Date(timeIntervalSince1970: TimeInterval(timeStamp!)).customTimeStr(customStr: "yyyy-MM-dd")
            }
        }
    }
